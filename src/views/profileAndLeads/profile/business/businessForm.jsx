...
class BusinessForm extends React.Component {
  ...
  componentDidMount() {
    ...
    if (id && id != undefined && id !== null && !this.props.createNewFormOpen) {
  ...
  componentDidUpdate = () => {
    if (
      ...
      if (Object.keys(this.props.businessInfo).length != 0) {
        for (let item in this.props.businessInfo) {
          if (formData[item] !== undefined) {
            formData[item].value = this.props.businessInfo[item];
          }
        }
  ...
  submitFn = async (e) => {
    ...
    if (pass) {
      ...
      if (this.state.isUpdateForm) {
        formData.append("id", this.props.businessInfo.id);
...
const mapStateToProps = (state, props) => {
  return {
    businessInfo: state.client.clientInfo,
  };
};
