...
import { getData, deleteData, filterData } from "redux/actions/clients";
...
const ActionsComponent = (props) => {
  return (
    <div className="d-flex data-list-action mt-1">
      <NavLink
        to={`/clients/${props.row.id}/edit`}
        className="d-inline text-dark fonticon-container"
      >
        <Edit className="cursor-pointer mr-1 mt-1 fonticon-wrap" size={20} />
      </NavLink>
      <div className="dropleft mr-1 mb-1 d-inline-block text-dark fonticon-container">
        <UncontrolledButtonDropdown direction="left">
          <DropdownToggle color="flat">
            <MoreHorizontal className="vx-icon fonticon-wrap" size={30} />
          </DropdownToggle>
          <DropdownMenu className="dropdown-menu">
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/invoice/create?client=${props.row.id}`);
              }}
            >
              Create Invoice
            </DropdownItem>
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/invoice?client=${props.row.id}`);
              }}
            >
              View Invoice
            </DropdownItem>
  ...
  return (
    <div className="data-list-header d-flex justify-content-between flex-wrap">
      <div className="actions-left d-flex flex-wrap">
        ...
        <NavLink to="/clients/create" className="d-inline">
      ...
      <div className="actions-right d-flex flex-wrap mt-sm-0 mt-2">
        ...
        <FormGroup className="width-200 mr-2">
          ...
          <Select
            ...
            defaultValue={filterClientOption[0]}
            name="color"
            onChange={(e) => props.handleFilterChange(e)}
            options={filterClientOption}
          />
        </FormGroup>
...
class ClientsList extends Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (
      props.clientList.data.length !== state.data.length ||
      state.currentPage !== props.parsedFilter.page
    ) {
      return {
        data: props.clientList.data,
        totalPages: props.clientList.totalPages,
        currentPage: parseInt(props.parsedFilter.page) - 1,
        rowsPerPage: parseInt(props.parsedFilter.perPage),
        totalRecords: props.clientList.totalRecords,
      };
    }
  ...
  state = {
    ...
    dataToExport: [],
    value: "",
    filterSelectValue: "business_name",
    modal: false,
    fileName: "",
    fileFormat: "xlsx",
    selectedRows: [],
  ...
  handleRowsPerPage = (value) => {
    ...
    history.push(`/clients?page=${page}&perPage=${value}`);
