...
export default class AddFamily extends Component {
    ...
    handleSubmit = (e) => {
      ...
      var formData = new FormData(document.querySelector("#family10"));
      let i=0,Name=[],Data=[];
      ...
      for(var j=0;j<i;j++ )
      {
        ...
        .then((response) => {
          if (response.data.status == 200) {
            ...
            this.props.history.push(`/customer/corporate/editemployee/${id}`);
          } else {
  ...
  async    componentDidMount() {
    let id=this.props.match.params.id;
    const induidual= await axios.get(`/api/individualcustomer/getById/${id}`);
    ...
    if(induidual.data.status==200)
    {
      this.setState(
        {
            induvidual_name:induidual.data.customer.individual_name,
...
getValuesFn = (data) => {
  ...
  render() {
    ...
    />
      ...
      <Card>
        ...
        <CardBody>
        <form onSubmit={this.handleSubmit} id="family10" >
          <Input
                    type="hidden"
 ...
 <small style={{color:"red"}}>{this.state.errorinsurence}</small>
            ...
            <Link to={`/customer/corporate/editemployee/${this.props.match.params.id}`}><Button id='corporatebackbtn' type='reset' value="Cancel" color='warning mr-2'>Cancel</Button></Link>
