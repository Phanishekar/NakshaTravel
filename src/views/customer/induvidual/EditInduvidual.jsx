...
import InduvidualFamilyList from './InduvidualFamilyList';
...
export default class EditInduvidual extends Component {
  constructor(props) {
    super(props);
  ...
  toggleActice= async(e)=>{
    ...
    const resp= await axios.get(`/api/individualcustomer/toggleActive/${id}`);
  ...
  handleSubmit = (e) => {
    ...
    var formData = new FormData(document.querySelector("#editindividual"));
    let i = 0, Name = [], Data = [];
  ...
  async componentDidMount() {
    ...
    if (res.data.status == 200) {
      this.setState(
        {
          individual_active:res.data.customer.individual_active,
          ...
          individual_billing_street_2: res.data.customer.individual_billing_street_2,
          ...
          individual_billing_pincode: res.data.customer.individual_billing_pincode,
          individual_billing_country: res.data.customer.individual_billing_country,
          individual_account_number:res.data.customer.individual_billing_country,
         individual_bank_name:res.data.customer.individual_bank_name,
          individual_account_code:res.data.customer.individual_account_code,
          individual_swift_code:res.data.customer.individual_swift_code,
            individual_currency:res.data.customer.individual_currency,
  ...
  render() {
    ...
    return (<React.Fragment>
      <BreadCrumbs
        ...
        breadCrumbActive="Edit Individual"
      />
      <Card>
        <CardHeader>
          <CardTitle>
          ...
          <div style={{display:"flex",justifyContent:"flex-end"}}>
                  {this.state.individual_active=="Active"?<Button id='corporatebackbtn' type='btn' onClick={this.toggleActice} value="Cancel" color='btn btn-success mr-2'>{this.state.individual_active}</Button>:<Button id='corporatebackbtn' type='btn' onClick={this.toggleActice} value="Cancel" color='btn btn-danger mr-2'>{this.state.individual_active}</Button>} </div>
        </CardHeader>
        <CardBody>


          <form onSubmit={this.handleSubmit} id="editindividual"  >
            <Input
              type="hidden"
...
<a href={"https://wellniverse.in/Naksha/api/" + this.state.individual_passport_attachment} target='blank'>{this.state.individual_passport_attachment}</a>}
            ...
            <hr />
                  ...
                  <Row>
                    <Col>
                        <FormGroup>
                        <Label for="individual_bank_name">Bank Name</Label>
                        <Input
                             type="text"
                             name="individual_bank_name"
                             value={this.state.individual_bank_name}
                             onChange={this.handleChange}
                      />
                        </FormGroup>
                    </Col>  
...
{this.state.errorinsurence?<small style={{color:"red"}}>{this.state.errorinsurence}</small>:
            ...
            <hr />
            <div style={{display:"flex",justifyContent:"flex-end"}}>
            <Link to={`/customer/induvidual`}><Button id='corporatebackbtn' type='reset' value="Cancel" color='warning mr-2'>Back</Button></Link>
      ...
      <InduvidualFamilyList id={this.props.match.params.id} nameq={this.state.individual_first_name} namew={this.state.individual_last_name}/>
