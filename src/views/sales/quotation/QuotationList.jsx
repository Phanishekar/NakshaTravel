...
export function DSRlist(props) {
  ...
  const getContry = async (pageNumber) => {
    try {
      ...
      const res = await axios.get(`api/sales/quotation/getAll?page=${pageNumber}`);
  ...
  const columns = [
    {
      ...
      name: <b>Quotation Number</b>,
      selector: (row) => "Q- " +row.dsr_number,
      width: "200px",
    },
    {
    ...
    
    {
      name:<b>Edit</b>,
      cell: (row) => (
        <Link to={`/addQuotation/${row.dsr_number}/form`}>
  ...
  return (
        ...
        breadCrumbActive="Quotation List"
      />
                 <Row>
...
<Col lg="4" >
<FormGroup>
<Link to="/sales/quotation/add"><Button  color='primary mr-2'>+ Add Quotations</Button> </Link>
</FormGroup>
</Col>
</Row>
