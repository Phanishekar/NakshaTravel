...
export function PaymentRecivedlist(props) {
  ...
  const getContry = async (pageNumber) => {
    try {
      ...
      const res = await axios.get(`/api/sales/payment/getAll?page=${pageNumber}`);
  ...
  const columns = [
    ...
    
    {
      name:<b>Edit</b>,
      cell: (row) => (
        <Link to={`/sales/EditPaymentRecived/${row.payment_received_id}`}>
  ...
  return (
        ...
        breadCrumbActive="Payment Recived"
      />
                 <Row>
...
<Col lg="4" >
<FormGroup>
<Link to="/sales/AddPaymentRecived"><Button  color='primary mr-2'>+ Add Payment Recived</Button> </Link>
</FormGroup>
</Col>
</Row>
