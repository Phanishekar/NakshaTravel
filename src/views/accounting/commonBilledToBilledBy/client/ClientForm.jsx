...
class ClientForm extends React.Component {
  ...
  componentDidUpdate = () => {
    if (
      ...
      if (Object.keys(this.props.editClientData).length != 0) {
        for (let item in this.props.editClientData) {
          if (formData[item] !== undefined) {
            formData[item].value = this.props.editClientData[item];
          }
        }
  ...
  componentDidMount() {
    ...
    const id = this.props.editClientData.id;
  ...
  submitFn = async (e) => {
    ...
    if (pass) {
      ...
      if (this.state.isUpdateForm) {
        formData.append("id", this.props.editClientData.id);
  ...
  render() {
    ...
    console.log("update form", formFields.show_email_in_invoice);
