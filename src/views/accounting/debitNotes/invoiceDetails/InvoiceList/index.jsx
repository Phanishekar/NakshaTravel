        ...
        <UncontrolledButtonDropdown direction="left">
          ...
          <DropdownMenu className="dropdown-menu">
            ...
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/debitNotes/${props.row.id}/edit`);
              }}
            >
...
class InvoiceList extends Component {
  ...
  handleCopyInvoice = async (row) => {
    let response = await Axios.post(
      `${REACT_APP_API_URL}/debitNotes/copy`,
      row
    );
    if (response.data.status == 1) {
      toast.success("Copied successfull");
      history.push("/debitNotes/" + response.data.data.id + "/edit");
    }
  };
  handleChangeStatus = async (obj, status) => {
    obj.status = status;
    let response = await Axios.post(
      `${REACT_APP_API_URL}/debitNotes/changeInvoiceStatus`,
      obj
    );
    if (response.data.status == 1) {
      toast.success("Status change successfull");
      this.props.getData(this.props.parsedFilter);
      history.push(`/debitNotes`);
    }
  };
  ...
  handleRowsPerPage = (value) => {
    ...
    history.push(`/debitNotes?${searchFilter}`);
