        ...
        <UncontrolledButtonDropdown direction="left">
          ...
          <DropdownMenu className="dropdown-menu">
            ...
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/purchaseOrders/${props.row.id}/edit`);
              }}
            >
...
class InvoiceList extends Component {
  ...
  handleCopyInvoice = async (row) => {
    let response = await Axios.post(
      `${REACT_APP_API_URL}/purchaseOrders/copy`,
      row
    );
    if (response.data.status == 1) {
      toast.success("Copied successfull");
      history.push("/purchaseOrders/" + response.data.data.id + "/edit");
    }
  };
  handleChangeStatus = async (obj, status) => {
    obj.status = status;
    let response = await Axios.post(
      `${REACT_APP_API_URL}/purchaseOrders/changeInvoiceStatus`,
      obj
    );
    if (response.data.status == 1) {
  ...
  handleRowsPerPage = (value) => {
    ...
    history.push(`/purchaseOrders?${searchFilter}`);
...
const mapStateToProps = (state) => {
  return {
    invoiceList: state.purchaseOrders,
  };
};
