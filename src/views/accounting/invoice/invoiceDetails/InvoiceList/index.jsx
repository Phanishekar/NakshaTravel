...
const ActionsComponent = (props) => {
  return (
    <div className="d-flex data-list-action">
      <div className="dropleft mr-1 d-inline-block">
        <UncontrolledButtonDropdown direction="left">
          ...
          <DropdownMenu className="dropdown-menu">
            ...
            <DropdownItem
              onClick={() => {
                props.row.copy = 0;
                props.row.invoice_type = "delivery_chalan";
                props.copyInvoice(props.row);
              }}
            >
              <Copy className="cursor-pointer mr-1" size={20} /> Generate
              Delivery Chalan
            </DropdownItem>
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/invoice/${props.row.id}/edit`);
              }}
            >
...
class InvoiceList extends Component {
  ...
  handleCopyInvoice = async (row) => {
    ...
    if (response.data.status == 1) {
      if ((response.data.data.invoice_type == "delivery_chalan")) {
        toast.success("Delivery Challan Generated");
        history.push("/deliveryChalan/" + response.data.data.id + "/edit");
      } else {
        toast.success("Copied successfull");
        history.push("/invoice/" + response.data.data.id + "/edit");
      }
    }
  };
  ...
  handleRowsPerPage = (value) => {
    ...
    history.push(`/invoice?${searchFilter}`);
