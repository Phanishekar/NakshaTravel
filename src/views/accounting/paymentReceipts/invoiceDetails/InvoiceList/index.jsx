...
import {
  ...
  deleteData,
  filterData,
} from "redux/actions/paymentReceipts";
        ...
        <UncontrolledButtonDropdown direction="left">
          ...
          <DropdownMenu className="dropdown-menu">
            ...
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/paymentReceipts/${props.row.id}/edit`);
              }}
            >
...
class InvoiceList extends Component {
  ...
  handleCopyInvoice = async (row) => {
    let response = await Axios.post(
      `${REACT_APP_API_URL}/paymentReceipts/copy`,
      row
    );
    if (response.data.status == 1) {
      toast.success("Copied successfull");
      history.push("/paymentReceipts/" + response.data.data.id + "/edit");
    }
  };
  handleChangeStatus = async (obj, status) => {
    obj.status = status;
    let response = await Axios.post(
      `${REACT_APP_API_URL}/paymentReceipts/changeInvoiceStatus`,
      obj
    );
    if (response.data.status == 1) {
      toast.success("Status change successfull");
      this.props.getData(this.props.parsedFilter);
      history.push(`/paymentReceipts`);
    }
  };
  ...
  handleRowsPerPage = (value) => {
    ...
    history.push(`/paymentReceipts?${searchFilter}`);
...
const mapStateToProps = (state) => {
  return {
    invoiceList: state.paymentReceipts,
  };
};
