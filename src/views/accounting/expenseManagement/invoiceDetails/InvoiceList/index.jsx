        ...
        <UncontrolledButtonDropdown direction="left">
          ...
          <DropdownMenu className="dropdown-menu">
            ...
            <DropdownItem
              tag="a"
              onClick={() => {
                history.push(`/expenseManagement/${props.row.id}/edit`);
              }}
            >
...
class InvoiceList extends Component {
  ...
  handleCopyInvoice = async (row) => {
    let response = await Axios.post(
      `${REACT_APP_API_URL}/invoices/copy`,
      row
    );
    if (response.data.status == 1) {
      toast.success("Copied successfull");
      history.push("/expenseManagement/" + response.data.data.id + "/edit");
    }
  };
  handleChangeStatus = async (obj, status) => {
    ...
    if (response.data.status == 1) {
      ...
      this.props.getData(this.props.parsedFilter);
      history.push(`/expenseManagement`);
    }
  };
  ...
  handleRowsPerPage = (value) => {
    ...
    history.push(`/expenseManagement?${searchFilter}`);
...
const mapStateToProps = (state) => {
  return {
    invoiceList: state.expenseManagement,
  };
};
